...
- Zeeberg, B. R. et al. [Mistaken identifiers: gene name errors can be introduced inadvertently when using Excel in bioinformatics.](https://bmcbioinformatics.biomedcentral.com/articles/10.1186/1471-2105-5-80) BMC Bioinformatics 5, 80 (2004).
...
#### Installing Stacks the hard way on a Mac {#installingstacksthehardwayonamac}

It really is much easier on Ubuntu (*sic*). Also, make sure the newer
versions of Stacks (1.40) do still require all the components below.

**First:**

1.  Install Xcode to get gcc <https://developer.apple.com/downloads/>
2.  Install Command Line Tools <https://developer.apple.com/downloads/>

**Second:**

1.  Install <http://dev.mysql.com/downloads/mysql/>
2.  Install pear
    <http://jason.pureconcepts.net/2012/10/install-pear-pecl-mac-os-x/>
3.  Install
    [http://search.cpan.org/\~timb/DBI–1.634/](http://search.cpan.org/~timb/DBI-1.634/)
4.  Install <http://search.cpan.org/dist/DBD-mysql/>

**Third:**

1.  Install <http://search.cpan.org/~jmcnamara/OLE-Storage_Lite/>
    because
    [http://search.cpan.org/\~jmcnamara/Spreadsheet-WriteExcel–2.37/](http://search.cpan.org/~jmcnamara/Spreadsheet-WriteExcel-2.37/)
    needs it to install
2.  [http://search.cpan.org/\~jmcnamara/Spreadsheet-WriteExcel–2.37/](http://search.cpan.org/~jmcnamara/Spreadsheet-WriteExcel-2.37/)

**Fourth:**

1.  To install samtools with version 0.1.19 that is required by Stacks,
    follow the advice in samtools README, but as it’s very poor and
    incomplete, follow this:
    [http://seqanswers.com/forums/archive/index.php/t–7132.html](http://seqanswers.com/forums/archive/index.php/t-7132.html).
    The relevant post is:

> Download the SAM tools\
> Unpack the SAM tools tarball and cd to the SAM tools source
> directory.\
> Build the SAM tools by typing make at the command line.\
> Choose a directory into which you wish to copy the SAM tools binary,
> the included library libbam.a, and the library headers. A common
> choice is /usr/local/. (I used /home/user/bin/samtools)\
> Copy libbam.a to the lib/ directory in the folder you’ve chosen above
> (e.g. /usr/local/lib/) - /home/user/bin/samtools/lib\
> Create a directory called “bam” in the include/ directory (e.g.
> /usr/local/include/bam) /home/user/bin/samtools/include/bam\
> Copy the headers (files ending in .h) to the include/bam directory
> you’ve created above (e.g. /usr/local/include/bam)\
> Copy the samtools binary to some directory in your PATH.

**Finally**

1.  Install Stacks as in the manual

#### MySQL commands {#mysqlcommands}

To start the server:

`bin/mysqld_safe &`

if user is mysql then:

`bin/mysqld_safe --user=mysql &`

To stop the server:

`bin/mysqladmin -u root -p shutdown` (password needed)
